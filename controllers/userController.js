...
const controller = {
  profile: function (req, res) {
    ...
    productos.findAll({ where: { userId: req.session.user.id } })
      .then(productos => {
       
        if (productos.length === 0) {
          return res.render("profile", { title: "Profile", productosUsuario: null });
        }
        return res.render("profile", { title: "Profile", productosUsuario: productos });
      })
